...
export const registerUser = asyncHandler(async (req, res) => {
  ...
  const existing = await User.findOne({
    $or: [{ email: email.toLowerCase() }, { username: username.toLowerCase() }], //Ha bármelyikre igaz akkor megtalálja
  });
...
//token és cookie nélküli verzió "DEMO"
export const logoutUser = asyncHandler(async (req, res) => {
  const isProd = process.env.NODE_ENV === "production";
  const refreshToken = req.cookies?.refreshToken;

  //DB-ből töröljük a refreshtokent ha van
...
export const refreshAccessToken = asyncHandler(async (req, res) => {
  ...
  // 4. SIKERES ÁG - ROTÁCIÓ (Ez már a try-catch-en KÍVÜL van)
